...
const btnLimparMes = document.querySelector("#btnLimparMes");
    ...
    //Seletores para os botões de filtro
    ...
    function inicializarSeletorDeMes() {
        ...
        //Seletor e evento para o botão de limpar o mês
        const btnLimparMes = document.querySelector("#btnLimparMes");
        ...
        btnLimparMes.addEventListener("click", () =>{
            ...
            if(confirmou){
                ...
                //Reseta o modo de edição caso estivesse ativo
                editandoIndex = null;
                btnSubmit.value = "Registrar Conta";
    ...
    frm.addEventListener("submit", (e) =>{
        ...
        if(editandoIndex !== null) {
            //Mantém o status de 'paga' ao editar
            contasDoMes[editandoIndex].descricao = descricao;
            contasDoMes[editandoIndex].valor = valor;
            editandoIndex = null;
            btnSubmit.value = "Registrar Conta"; //Muda o texto do botão de volta

            btnSubmit.classList.remove("modo-edicao");
        }else{
            //Adiciona a propriedade 'paga: false' para novas contas 
        contasDoMes.push({ descricao, valor, paga: false });
}
        //Chama a função para atualizar a tela com os novos dados
        renderizarContasDoMes();
        salvarDados();  //Salva a nova lista após adicionar

        //Limpa os campos e coloca o foco no primeiro campo
        frm.reset();
        frm.inDescricao.focus();
    });
    ...
    listaContasEl.addEventListener("click", (e) =>{
        ...
        if(target.classList.contains("btn-excluir")) {
            ...
            //Coloca os dados da conta no formulário
            frm.inDescricao.value = contaParaEditar.descricao;
            frm.inValor.value = contaParaEditar.valor;

            //Guarda o índice do item que estamos editando
            editandoIndex = index;

            //Muda o texto do botão para indicar que estamos salvando
            btnSubmit.value = "Salvar Alterações";

            //Coloca o foco no campo de descrição para facilitar a edição
            frm.inDescricao.focus();
            //Não precisa salvar nem renderizar aqui.só no submit
